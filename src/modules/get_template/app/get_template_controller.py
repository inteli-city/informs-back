...
from src.shared.helpers.contracts.runtime_requests import GetTemplateControllerRequestSchema
...
class GetTemplateController:
    ...
    def __call__(self, request: IRequest) -> IResponse:
        try:
            ...
            payload = GetTemplateControllerRequestSchema.model_validate(data)
            requester = UserGatewayDTO.from_api_gateway(payload.requester_user.model_dump(by_alias=True))

            template = self.usecase(template_id=payload.template_id, requester=requester)
